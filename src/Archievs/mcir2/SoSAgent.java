...
public class SoSAgent extends Agent {
    ...
    HashMap<String, String> decisionlookuptbl = new HashMap<String, String>();// <ack/nack, ldz id> then sosagent will subtract one from the total size
...
//    WriteToFile wtf = new WriteToFile();
    ...
    int ackActionCounter = 0, nackActionCounter = 0;
    boolean lock = false;
    int lost = 0;
...
//        wtf.writeToExcel(rprt);
        ...
        System.out.println(decisionlookuptbl);
    }
...
//                    HashMap<String, String> decision = new HashMap<String, String>();
                    ...
                    Iterator<Map.Entry<String, String>>
                            iterator = decisionlookuptbl.entrySet().iterator();
                    while (iterator.hasNext()) {
                        Map.Entry<String, String> entry = iterator.next();
                        if (entry.getKey().equals(csresp.get(2).trim())) {
                            flg = true;
                            if (!entry.getValue().equals(csresp.get(0).trim())) {
                                ackActionCounter++;// why not nackCount++, just because flipping is only possible from nnack to ack
                                decisionlookuptbl.put(csresp.get(2).trim(), csresp.get(0).trim());
                            }
                        }

                    }
                    if (flg == false) {
                        if (csresp.get(0).equals("ACK")) {
                            ackActionCounter++;
                        } else {
                            nackActionCounter++;
                        }
                    ...
                    info.add(ackActionCounter + "");
                    info.add("action");
                    info.add(decisionlookuptbl.size() + "");

                    reply.setContent(info.toString());
                    System.out.println("Remaining patients: " + scenario.getPatientNumber());
